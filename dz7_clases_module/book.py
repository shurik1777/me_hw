...
2. Класс Journal (журнал) представляет собой коллекцию студентов и предметов.
 У него есть методы для добавления студента, добавления оценки студенту по предмету,
 вывода ведомости студента и вывода средней оценки по предмету.
3. Для получения средней оценки по всем предметам используется
метод get_average_grade класса Student, который принимает опциональный аргумент subject (предмет),
чтобы получить среднюю оценку только по определенному предмету.
